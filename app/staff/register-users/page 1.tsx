    ...
    .map(
      (student) => `
        <tr>
          ...
          <td>${escapeHtml(student.approved === false ? t.notApproved : t.approved)}</td>
        </tr>`,
    )
    .join("");
...
export default function RegisterUserDataAddPage() {
  ...
  const [name, setName] = useState("AungPyaeSone");
  const [fatherName, setFatherName] = useState("U Soe Thaung");
  const [rollNumber, setRollNumber] = useState("5CS-36");
  const [graduatedYear, setGraduatedYear] = useState("2025");

  const [nrcData, setNrcData] = useState<NrcItem[]>([]);
  const [nrcRegion, setNrcRegion] = useState("7");
  const [nrcCode, setNrcCode] = useState("မညန");
  const [nrcType, setNrcType] = useState("(နိုင်)");
  const [nrcNumber, setNrcNumber] = useState("၁၁၄၀၀၀");
  ...
  async function loadNrcData() {
    try {
      ...
      const rows: NrcItem[] = Array.isArray(json?.data)
        ? json.data.map(normalizeNrcRow).filter((item: NrcItem) => item.name_mm)
        : [];

      setNrcData(rows);

      const defaultTownship = rows.find(
      ...
      if (defaultTownship) {
        ...
        setNrcRegion(rows[0].nrc_code);
        setNrcCode(rows[0].name_mm);
      }
    } catch {
  ...
  function exportTemplate() {
    const rows = [
      {
        name: "AungPyaeSone",
        fatherName: "U Soe Thaung",
        rollNumber: "5CS-36",
        graduatedYear: 2026,
        nrc: "၇/မညန(နိုင်)၁၁၄၀၀၀",
      },
    ];
  ...
  function exportCurrentData() {
    const rows = filteredStudents.map((student) => ({
      ...
      nrc: student.nrc,
      approved: student.approved === false ? t.notApproved : t.approved,
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/50">
                    ...
                    {selectedTownship?.city_mm && (
                      <p className="mt-2 text-xs font-bold text-[#008B8B]">
                        {t.cityHint} - {selectedTownship.city_mm}
                      </p>
                    )}
                            ...
                            >
                              ...
                              <td className="px-5 py-4">
                                <StatusBadge
                                  ...
                                  approvedText={t.approved}
                                  notApprovedText={t.notApproved}
                                />
                              </td>
                              <td className="px-5 py-4">
                      ...
                      {paginatedStudents.map((student) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <p className="mt-2 text-xs font-bold text-slate-600 dark:text-slate-300">
                                {student.rollNumber} • {student.graduatedYear}
                              </p>
                          ...
                          <p className="mt-3 text-xs font-bold text-slate-400">
                            {t.created}: {formatDate(student.createdAt)}
                          </p>
