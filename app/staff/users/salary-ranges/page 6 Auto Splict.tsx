  ...
  const sectionsHtml = chunks
    .map((chunk, index) => {
      const chartHtml = chunk
        .map((item) => {
          const minHeight = Math.max((item.minSalary / globalMax) * BAR_MAX_HEIGHT, item.minSalary ? BAR_MIN_HEIGHT : 6);
          const maxHeight = Math.max((item.maxSalary / globalMax) * BAR_MAX_HEIGHT, item.maxSalary ? BAR_MIN_HEIGHT : 6);
...
export default async function AdminSalaryRangesPage({
  ...
  // Chunk items into sets of max 6 per graph view
  const salaryChunks = chunkArray(salaryItems, 6);
      ...
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1 px-4 pb-8 pt-16 sm:px-6 md:px-8 lg:pt-8">
          <div className="mx-auto max-w-7xl space-y-4 md:space-y-6">
            ...
            {/* Render Multiple Chunks / Graphs Automatically if > 6 items */}
                ...
                <div key={groupIndex} className="space-y-4 md:space-y-6">
                  ...
                  <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800/80 dark:bg-slate-900/50">
                    ...
                    <div className="overflow-x-auto overflow-y-hidden p-3 sm:p-5 md:p-6">
                      <div className="relative min-w-[1200px] rounded-2xl p-3 sm:p-5">
