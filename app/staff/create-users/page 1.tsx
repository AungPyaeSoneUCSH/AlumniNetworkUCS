...
export default function AdminCreateUserPage() {
  ...
  }, [students, listFilter, searchQuery]);
    ...
    try {
      ...
      const usersToImport = rows
        .map((row) => ({
          ...
          password: cleanText(row.password || row.Password),
        }))
        .filter(
          (user) =>
            user.name &&
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full text-left text-sm">
                    <thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-400 dark:bg-slate-900/80 dark:text-slate-500">
                      <tr>
                        <th className="px-5 py-3.5 font-black">{t.name}</th>
                        <th className="px-5 py-3.5 font-black">{t.fatherName}</th>
                        <th className="px-5 py-3.5 font-black">{t.graduatedYear}</th>
                        <th className="px-5 py-3.5 font-black">{t.status}</th>
