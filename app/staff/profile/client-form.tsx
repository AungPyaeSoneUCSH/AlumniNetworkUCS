// file: app/staff/profile/client-form.tsx
...
const text = {
  en: {
    ...
    subtitle: "Update your staff account details and security credentials.",
    name: "Staff Name",
    email: "Staff Email",
    position: "Position",
  ...
  mm: {
    ...
    subtitle: "သင့် staff အကောင့်အချက်အလက်များနှင့် လုံခြုံရေးကို ပြင်ဆင်ပါ။",
    name: "အမည်",
    email: "အီးမေးလ်",
    position: "ရာထူး",
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/staff/profile", {
        ...
        body: JSON.stringify({
          name: name.trim(),
          email: user.email, // Email is unchanged since it is disabled
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="group relative">
        <div className={`pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 transition ${
          disabled ? "text-slate-400/60 dark:text-slate-600" : "text-slate-400 group-focus-within:text-[#008B8B]"
        }`}>
          {icon}
        </div>
        <input
          ...
          className={`h-12 w-full rounded-xl border pl-12 pr-4 text-sm font-bold outline-none transition placeholder:text-slate-400 
            ${disabled 
              ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-500 dark:border-slate-800/80 dark:bg-slate-900/60 dark:text-slate-500" 
              : "border-slate-200 bg-slate-50 text-slate-900 focus:border-[#00BFC4] focus:bg-white focus:ring-4 focus:ring-cyan-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:border-[#00BFC4] dark:focus:bg-slate-900 dark:focus:ring-cyan-900/30"
            }`}
        />
      </div>
    </div>
