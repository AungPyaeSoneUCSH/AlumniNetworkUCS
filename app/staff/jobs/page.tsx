...
export default async function StaffJobsPage({
  ...
  // Only allow normal alumni users (role === "user") to show up in jobs management
  ...
  const makePageHref = (pageNumber: number) => {
    ...
    return `/staff/jobs?${params.toString()}`;
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 text-slate-950 dark:bg-slate-950 dark:text-white">
      <div className="flex min-h-screen">
        <StaffSidebar active="jobs" lang={lang} />
