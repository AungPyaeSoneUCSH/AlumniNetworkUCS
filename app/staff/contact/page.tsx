// file: app/staff/contact/page.tsx
...
const text = {
  en: {
    ...
    save: "Save Changes",
    saving: "Saving...",
    loadFailed: "Failed to load contact info (Ensure backend allows Staff).",
  ...
  mm: {
    ...
    save: "သိမ်းမည်",
    saving: "သိမ်းနေသည်...",
    loadFailed: "ဆက်သွယ်ရန် အချက်အလက်များ မဖတ်နိုင်ပါ။ (Staff permission လိုအပ်နိုင်ပါသည်)",
...
function StaffContactContent() {
  ...
  useEffect(() => {
    ...
    if (status === "unauthenticated") {
      router.replace(`/staff/login?lang=${currentLang}`);
      return;
    }
    ...
    async function loadContact() {
      ...
      try {
        // Verify staff access locally to prevent UI glitching
        const roleRes = await fetch("/api/staff/me", { cache: "no-store" });
        if (!roleRes.ok) {
          router.replace(`/staff/login?lang=${currentLang}`);
          return;
        }
        ...
        if (!res.ok) {
          setIsError(true);
          setMessage(res.status === 403 ? "Forbidden: You don't have permission to view this." : t.loadFailed);
          return;
        }
  ...
  }, [status, router, currentLang, t.loadFailed]);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        setIsError(true);
        setMessage(res.status === 403 ? "Forbidden: Staff may not have permission to edit contacts." : t.updateFailed);
        return;
      }
