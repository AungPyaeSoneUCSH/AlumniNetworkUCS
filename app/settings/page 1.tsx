...
function buildSalaryOptions() {
  ...
  for (let start = 300000; start < 10000000; start += 200000) {
    const end = start + 200000;
    options.push(`${start.toLocaleString()} - ${end.toLocaleString()}`);
  }

  options.push("Upper 10,000,000");
  return options;
}
...
export default function SettingsPage() {
  ...
  const yearOptions = useMemo(() => {
    const years = [];
    for (let year = currentYear; year >= 2000; year--) years.push(year);
    return years;
  }, [currentYear]);
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(v) => updateField("department", v as Degree)}
                    >
                      <option value="">{t.selectDegree}</option>
                      {degrees.map((degree) => (
                        <option key={degree || "empty"} value={degree}>
                          {degree || t.selectDegree}
                        </option>
                      ))}
                    </Select>
                  </div>
          ...
          <Card title={t.socialLinks}>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <SocialInput
                ...
                value={form.socialLinks?.viber || ""}
                onChange={(v) => updateSocialField("viber", v)}
              />
              <SocialInput
                icon={<FaWhatsapp />}
                label={t.whatsapp}
                ...
                onChange={(v) => updateSocialField("whatsapp", v)}
              />
              {/* <SocialInput icon={<FaXTwitter />} label={t.x} prefix={socialPrefixes.x} value={form.socialLinks?.x || form.socialLinks?.twitter || ""} onChange={(v) => updateSocialField("x", v)} /> */}
