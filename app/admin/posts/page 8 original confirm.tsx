...
import {
  ...
  FileText,
  Globe2,
} from "lucide-react"; // CHANGED: Added dropdown and export icons
...
// CHANGED: Imported PrintUsersButton to handle generic HTML printing logic, removed external PostsPrintExport
...
// CHANGED: Added CSV data URL generation specific to Posts
function csvDataUrl(posts: any[], t: typeof text.en) {
  const rows = makeExportRows(posts, t);
  const header = [t.author, t.email, t.category, t.content, t.likes, t.comments, t.date];
  const csvRows = [header, ...rows.map((r) => [r.author, r.email, r.category, r.content, r.likes, r.comments, r.date])];
...
export default async function AdminPostsPage({
  ...
  // CHANGED: Export Data URLs definitions
  ...
  const printHtml = exportDocumentHtml(filteredPosts, t, false);
          ...
          <div className="mx-auto max-w-7xl space-y-4 md:space-y-6">
            ...
            <div className="relative z-20 overflow-visible rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/50 sm:p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="relative z-50 flex w-full flex-wrap items-center gap-2 overflow-visible lg:w-auto lg:justify-end">
                  {/* CHANGED: Replaced `<PostsPrintExport />` with inline dropdown component */}
                  <details className="group relative z-[200] inline-flex overflow-visible">
                    <summary className="flex h-9 cursor-pointer list-none items-center gap-2 rounded-xl bg-gradient-to-r from-[#25C9C8] to-[#008B8B] px-4 py-2 text-xs font-black text-white shadow-md shadow-cyan-500/20 transition-all hover:scale-[1.02] hover:brightness-110 active:scale-95 marker:hidden [&::-webkit-details-marker]:hidden">
                      <Download size={15} />
                      {t.export}
              ...
              <form
                ...
                action="/admin/posts"
                className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-[1.4fr_170px_170px_150px_150px]"
              >
                ...
                <label className="relative block md:col-span-2 xl:col-span-1">
                  ...
                  <input
                    ...
                    className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-xs font-bold text-slate-800 outline-none transition focus:border-[#25C9C8] focus:ring-2 focus:ring-[#25C9C8]/20 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:border-[#25C9C8]"
                  />
                </label>
                ...
                <input
                  ...
                  className="h-10 w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-bold text-slate-700 outline-none transition focus:border-[#25C9C8] focus:ring-2 focus:ring-[#25C9C8]/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
                />
              ...
              <Script id="posts-filter-auto-submit" strategy="afterInteractive">
                {`
                  (() => {
                    ...
                    if (document.readyState === "loading") {
                      document.addEventListener("DOMContentLoaded", bindAutoSubmit, { once: true });
                    } else {
                      bindAutoSubmit();
                    }
                  })();
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                    {paginatedPosts.map((post) => {
                      ...
                      return (
                        <tr
                          key={String(post._id)}
                          className="transition-colors hover:bg-cyan-50/40 dark:hover:bg-[#25C9C8]/10"
                        >
                          ...
                          <td className="px-4 py-3.5 text-xs font-black text-slate-600 dark:text-slate-400">
              ...
              {paginatedPosts.map((post) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-4 line-clamp-4 text-xs font-semibold leading-5 text-slate-600 dark:text-slate-300">
                      {post.content || t.noContent}
                    </p>
...
// CHANGED: Added `ExportItem` component to render dropdown options cleanly 
function ExportItem({
  href,
  fileName,
  icon,
...
function SelectBox({
  ...
  return (
    <select
      ...
      className="h-10 w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-bold text-slate-800 outline-none transition focus:border-[#25C9C8] focus:ring-2 focus:ring-[#25C9C8]/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:focus:border-[#25C9C8]"
    >
      {children}
    </select>
  );
...
function AuthorCell({ author, t }: { author: any; t: typeof text.en }) {
  return (
    <div className="flex min-w-0 items-center gap-3">
      ...
      <div className="min-w-0">
        <p className="line-clamp-1 text-xs font-black text-slate-950 dark:text-white">
          {author.name || t.unknownAlumni}
        </p>
        <p className="mt-0.5 line-clamp-1 text-[11px] font-bold text-slate-400 dark:text-slate-500">
...
function PostActions({
  ...
  return (
    <div
      className={`flex gap-2 ${mobile ? "grid grid-cols-1" : "justify-end"}`}
    >
...
function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex rounded-full bg-[#E0FAFA] px-2.5 py-1.5 text-[11px] font-black text-[#008B8B] dark:bg-[#25C9C8]/10 dark:text-cyan-300">
      {children}
    </span>
  );
}
...
function MiniInfo({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="min-w-0 rounded-xl bg-slate-50 p-2.5 dark:bg-slate-950/50">
      ...
      <p className="mt-1 line-clamp-2 text-xs font-black text-slate-800 dark:text-slate-200">
        {value}
      </p>
    </div>
  );
...
function EmptyPosts({ t }: { t: typeof text.en }) {
  return (
    <div className="p-10 text-center">
      ...
      <h2 className="mt-4 text-lg font-black text-slate-900 dark:text-white">{t.noPosts}</h2>
  ...
  return (
    <Link
      ...
      className={`flex h-9 min-w-[36px] items-center justify-center rounded-xl px-3 text-xs font-black transition-all active:scale-95 ${
        active
          ? "bg-gradient-to-r from-[#25C9C8] to-[#008B8B] text-white shadow-md shadow-cyan-500/20"
