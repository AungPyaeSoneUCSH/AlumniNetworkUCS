...
import { exportTableHtml } from "@/components/admin/export-graph-buttons";
...
<head>
  ...
  <style>
    *{box-sizing:border-box}
    ...
    @media print{body{background:#fff;padding:0}.sheet{border:0;border-radius:0}.chart-wrap{break-inside:avoid}button{display:none}}
  </style>
</head>
<body>
  <div class="sheet">
    ...
    ${graphHtml}
    ${tableHtml.replace(/<!doctype html>[\s\S]*?<body>\s*<h1>[\s\S]*?<\/h1>/, "").replace(/<\/body>\s*<\/html>/, "")}
  </div>
</body>
</html>`;
}
  ...
  if (kind === "salary") {
    ...
    return `<div class="chart-wrap">
      ...
      <div class="chart">
        ${salaryItems
          .map((item) => {
            const minHeight = Math.max((item.minSalary / max) * 245, item.minSalary ? 26 : 6);
            const maxHeight = Math.max((item.maxSalary / max) * 245, item.maxSalary ? 26 : 6);
    ...
    return `<div class="chart-wrap">
      ...
      <div class="chart">
        ${employmentItems
          .map((item) => {
            const values = [
              { value: item.totalPercent, count: item.totalCount, color: "#0b67a3" },
              { value: item.employedPercent, count: item.employedCount, color: "#35a4df" },
              { value: item.unemployedPercent, count: item.unemployedCount, color: "#86e4f5" },
            ];
            return `<div class="bar-group">
              ${values
                .map((bar) => {
                  const height = Math.max((bar.value / 100) * 245, bar.value ? 22 : 6);
        ...
        <section className="min-w-0 flex-1 p-4 pt-20 sm:p-6 sm:pt-20 lg:p-8">
          <div className="mx-auto max-w-[1600px] space-y-5">
            <div className="grid gap-5 xl:grid-cols-2">
              <VerticalGraphCard
                ...
                selectBox={
                  <form className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                    ...
                    <button className="rounded-2xl bg-[#0b67a3] px-4 py-3 text-sm font-black text-white transition hover:opacity-90">
                      {t.apply}
                    </button>
              ...
              <SalaryGraphCard
                title={salaryTitle}
                subtitle={t.subtitleSalary}
                items={salaryItems}
                emptyText={t.noSalary}
                csvContent={exportCsv(salaryItems, salaryTitle, t)}
                ...
                selectBox={
                  <form className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                    ...
                    <button className="rounded-2xl bg-[#35a4df] px-4 py-3 text-sm font-black text-white transition hover:opacity-90">
                      {t.apply}
                    </button>
              ...
              <div className="xl:col-span-2">
                <EmploymentGraphCard
                  title={jobTitle}
                  subtitle={t.subtitleJob}
                  items={employmentItems}
                  emptyText={t.noJob}
                  csvContent={exportCsv(employmentItems, jobTitle, t)}
                  ...
                  selectBox={
                    <form className="grid gap-3 sm:grid-cols-[1fr_1fr_auto_auto]">
                      ...
                      <button className="rounded-2xl bg-[#0b67a3] px-5 py-3 text-sm font-black text-white transition hover:opacity-90">
                        {t.apply}
                      </button>
