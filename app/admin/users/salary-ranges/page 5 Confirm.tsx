...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .chart {
    height: 310px;
    /* Wide canvas so bars never get squeezed */
    min-width: 1200px; 
    display: flex;
...
<body>
  ...
  <div class="chart-scroll">
    ...
    ${items
      .map((item) => {
        const minHeight = Math.max((item.minSalary / max) * BAR_MAX_HEIGHT, item.minSalary ? BAR_MIN_HEIGHT : 6);
        const maxHeight = Math.max((item.maxSalary / max) * BAR_MAX_HEIGHT, item.maxSalary ? BAR_MIN_HEIGHT : 6);
          ...
          const downloadPdfFile = async () => {
            ...
            try {
              if (toggle) {
                ...
                // Temporarily disable while generating
