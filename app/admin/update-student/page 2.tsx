...
export default async function UpdateStudentPage({
  ...
  async function updateUserAction(
    ...
    try {
      // Fetch the existing user to get their current hashed password
      const existingUser = await User.findById(userId).select("email password");
      if (!existingUser) return { error: "User not found." };

      // Check for duplicate emails
      const emailTaken = await User.findOne({ email: data.email, _id: { $ne: userId } });
      ...
      // Check the new password against the old password
      if (data.password) {
        if (existingUser.password) {
          const isSamePassword = await bcrypt.compare(data.password, existingUser.password);
          if (isSamePassword) {
            return { error: lang === "mm" ? "စကားဝှက်အသစ်သည် အဟောင်းနှင့် တူညီနေပါသည်။" : "The new password cannot be the same as the old password." };
          }
        }
        
        // Hash the new password and apply the update
