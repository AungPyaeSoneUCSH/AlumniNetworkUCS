...
const text = {
  en: {
    ...
    pageSubtitle:
      "Filter, export, view, block, unblock, and delete user accounts.",
    searchPlaceholder: "Search name, email, degree...",
  ...
  mm: {
    ...
    pageSubtitle:
      "အသုံးပြုသူများကို ရှာဖွေ၊ export ထုတ်၊ ကြည့်ရှု၊ block / unblock နှင့် delete ပြုလုပ်နိုင်သည်။",
    searchPlaceholder: "နာမည်၊ email၊ degree ဖြင့် ရှာရန်...",
...
function exportRows(users: any[], t: typeof text.en) {
  return [
    [t.name, t.email, t.degree, t.graduatedYear, t.status],
    ...users.map((user) => [
    ...
    .map(
      (user) => `
        <tr>
          ...
          <td>${escapeHtml(user.isBlocked ? t.blocked : t.active)}</td>
        </tr>`
    )
    .join("");
...
function pdfDataUrl(users: any[], title: string, t: typeof text.en) {
  ...
  const tableTop = 492;
  const rowHeight = 24;
  const colWidths = [145, 250, 135, 110, 100];
  const tableWidth = colWidths.reduce((sum, width) => sum + width, 0);

  const headerRow = [t.name, t.email, t.degree, t.graduatedYear, t.status];
  ...
  const pageStreams = pages.map((pageRows, pageIndex) => {
    ...
    pageRows.forEach((row, rowIndex) => {
      row.forEach((cell, colIndex) => {
        ...
        const maxChars = [23, 42, 22, 16, 14][colIndex];
...
export default async function AdminManageUsersPage({
  ...
  const allUsers: any[] = await User.find({})
    .sort({ createdAt: -1 })
    .select(
      "_id name email image role isBlocked degree department graduatedYear createdAt"
    )
    .lean();
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-end gap-2">
                          <Link
                            href={`/admin/users/${user._id}?lang=${lang}`}
                          ...
                          <form action={updateUserStatus}>
                            ...
                            <button
                              name="status"
                              value={user.isBlocked ? "active" : "blocked"}
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">
                  ...
                  <form action={updateUserStatus}>
                    ...
                    <button
                      name="status"
                      value={user.isBlocked ? "active" : "blocked"}
                      className={`w-full rounded-xl px-3 py-2 text-xs font-black text-white ${
                        user.isBlocked ? "bg-emerald-500" : "bg-amber-500"
                      }`}
                    >
                      {user.isBlocked ? t.unblock : t.block}
                    </button>
                  ...
                  <form action={deleteUserAccount}>
                    <input type="hidden" name="id" value={String(user._id)} />
                    <button className="w-full rounded-xl bg-red-500 px-3 py-2 text-xs font-black text-white">
                      {t.delete}
                    </button>
                  </form>
                </div>
