...
import ApprovedStudent from "@/models/ApprovedStudent"; // Updated to use ApprovedStudent model
...
async function deleteUserAccount(formData: FormData) {
  ...
  // 1. Fetch user to obtain key matching details (name and graduatedYear)
  const userToDelete: any = await User.findById(id).lean();

  if (userToDelete) {
    // 2. Unregister matching student record based ON NAME AND GRADUATED YEAR
    if (userToDelete.name) {
      // Escape special characters and trim to prevent space/formatting mismatches
      const safeName = userToDelete.name.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      
      const queryFilter: any = {
        name: new RegExp(`^\\s*${safeName}\\s*$`, "i"), // Case-insensitive exact match with optional spaces
      };

      // Only add graduatedYear to the filter if the user account actually has it
      if (userToDelete.graduatedYear) {
        queryFilter.graduatedYear = Number(userToDelete.graduatedYear);
      }

      // Find the ApprovedStudent by Name & Year and set registered to false
  ...
  // 4. Force UI refresh for all related pages to show updated status immediately
  revalidatePath("/admin/manage-users");
  revalidatePath("/admin/register-users");
            ...
            <div className="relative z-20 overflow-visible rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/50 sm:p-5">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                ...
                <div className="relative z-50 flex w-full flex-wrap items-center gap-2 overflow-visible xl:w-auto xl:justify-end">
                  ...
                  {/* CHANGED: Removed standalone export buttons here. Kept only the dropdown. */}
...
// CHANGED: We can leave ExportButton and ExportItem unchanged as ExportItem is still used in the details block. 
