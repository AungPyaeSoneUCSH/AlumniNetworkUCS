// app/admin/register-users/page.tsx
"use client";
...
type Student = {
  ...
  createdAt: string;
  registered?: boolean; // Changed from approved to registered
};
    ...
    .map(
      (student) => `
        <tr>
          ...
          <td>${escapeHtml(student.registered ? t.registered : t.notRegistered)}</td>
  ...
  const filteredAndSortedStudents = useMemo(() => {
    ...
    if (sortConfig !== null) {
      filtered.sort((a, b) => {
        ...
        // Handle the synthetic 'status' key sorting based on 'registered'
              ...
              <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={toggleView}
                    className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#00BFC4] to-[#008B8B] px-4 py-2 text-xs font-black text-white shadow-sm transition hover:scale-[1.02] hover:brightness-110 active:scale-95 sm:text-sm"
                  >
            ...
            <div className="grid gap-4 md:gap-6">
              {showAddForm ? (
                ...
                <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800/80 dark:bg-slate-900/50">
                  <div className="flex flex-col justify-between gap-3 border-b border-slate-200/80 p-4 dark:border-slate-800/60 sm:flex-row sm:items-center sm:p-5">
                    <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 text-xs font-bold outline-none transition focus:border-[#00BFC4] focus:ring-2 focus:ring-[#00BFC4]/15 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:border-[#00BFC4] sm:w-96"
                      />
                    </div>
                  </div>
...
function ActionButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#00BFC4] to-[#008B8B] px-4 py-2 text-xs font-black text-white shadow-sm transition hover:scale-[1.02] hover:brightness-110 active:scale-95 sm:text-sm [&_svg]:h-4 [&_svg]:w-4"
    >
      {icon}
      {children}
    </button>
