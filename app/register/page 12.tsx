...
// UPGRADE COMMENT: This function completely strips all spaces (\s+) from a string and lowercases it. 
// It guarantees that "၁၄/ကခန (နိုင်) ၁၁၈၉၅၃" and "၁၄/ကခန(နိုင်)၁၁၈၉၅၃" become exactly the same.
...
export default function RegisterPage() {
  ...
  const nrcCandidates = useMemo(() => {
    ...
    const generateSpacingVariations = (r: string, n: string) => [
      `${r}/${c}${t}${n}`,           // No spaces
      `${r}/${c}${t} ${n}`,          // Space before number
      `${r}/${c} ${t}${n}`,          // Space before type
      `${r}/${c} ${t} ${n}`,         // Space around type
      `${r}/ ${c}${t}${n}`,          // Space before code
      `${r} /${c}${t}${n}`,          // Space before slash
      `${r} / ${c} ${t} ${n}`,       // Spaced everywhere
    ];
    ...
    try {
      const res = await fetch("/api/register/check-approved", {
        ...
        body: JSON.stringify({
          ...
          // UPGRADE COMMENT: We now send a perfectly stripped version of the NRC strings. 
          // Your backend API MUST use these fields to do the check.
          // Example backend check: 
          // if (db_nrc.replace(/\s+/g, '') === req.body.normalizedNrcMm) { return true }
    ...
    try {
      const res = await fetch("/api/register", {
        ...
        body: JSON.stringify({
          ...
          // UPGRADE COMMENT: Include stripped strings here as well for final registration.
          normalizedNrcMm: normalizeForMatch(nrcValueMm),
          normalizedNrcEn: normalizeForMatch(nrcValueEn),
