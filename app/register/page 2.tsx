  ...
  return (
    <section className="mm relative flex min-h-screen items-center justify-center overflow-hidden bg-[#94EFEE] px-4 py-10 text-slate-950">
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-white/60 bg-white/80 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
        ...
        {step === "info" && (
          <>
            <h1 className="relative mb-2 text-center text-3xl font-black tracking-tight text-slate-950">
              {lang === "mm" ? "အကောင့်ဖန်တီးမည်" : "Create Account"}
            </h1>

            <p className="mb-7 text-center text-sm font-bold text-slate-500">
              {lang === "mm"
                ? "Admin ထည့်ထားသော အမည်နှင့် မှတ်ပုံတင်အမှတ် ကိုက်ညီမှသာ Register လုပ်နိုင်ပါသည်။"
                : "You can register only if your name and register number match admin approved data."}
            </p>
            ...
            <form onSubmit={register} className="relative space-y-5">
              <Input
                ...
                placeholder={
                  lang === "mm"
                    ? "Admin data နှင့် တူညီသောအမည်"
                    : "Name must match admin data"
                }
              ...
              <div className="space-y-2">
                ...
                {showNumberError && <FieldError message={numberError} />}
              ...
              <button
                ...
                className="flex h-13 w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3.5 text-sm font-black tracking-wide text-white shadow-lg transition hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50"
              >
                {approvalChecking ? (
