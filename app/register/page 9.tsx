...
export default function RegisterPage() {
  ...
  const nrcCandidates = useMemo(() => {
    ...
    return [
      nrcValueMm,
      nrcValueEn,
      `${enToMmDigit(nrcRegion)}/${nrcCode}${nrcType}${mmToEnDigit(nrcNumber)}`,
      `${nrcRegion}/${nrcCode}${nrcType}${nrcNumber}`,
    ];
  }, [nrcValueMm, nrcValueEn, nrcRegion, nrcCode, nrcType, nrcNumber]);
  ...
  return (
    <main className="min-h-[calc(100vh-60px)] px-2 pb-6 pt-6 sm:px-3">
      <section className="relative mx-auto grid max-w-7xl overflow-hidden rounded-2xl border border-white/20 shadow-md lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950/86 via-slate-950/58 to-slate-950/28" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_16%_18%,rgba(241,205,114,0.24),transparent_34%),radial-gradient(circle_at_82%_20%,rgba(20,184,166,0.28),transparent_36%)]" />
        ...
        <div className="relative z-10 flex items-center px-5 pb-8 sm:px-8 lg:px-10 lg:py-10">
          <div className="animate-form w-full rounded-[2rem] border border-white/30 bg-white/92 p-5 shadow-2xl backdrop-blur-2xl sm:p-7">
            <div className="mb-6 grid grid-cols-3 gap-2 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-2">
            ...
            {step === "approval" && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {nrcValueMm && (
                    <p className="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-sm font-black text-emerald-700">
                      {t.nrcPreview}: {nrcValueMm}
                    </p>
                  )}

                  {selectedTownship?.city_mm && (
                    <p className="mt-2 rounded-xl bg-[#eaffff] px-3 py-2 text-sm font-black text-[#008B8B]">
                ...
                <button
                  ...
                  className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 px-5 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-60"
                >
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <button
                    ...
                    className="h-11 rounded-xl border border-emerald-200 bg-white px-5 text-sm font-black text-emerald-700 transition hover:bg-emerald-50"
                  >
                    {t.back}
                  </button>
                  ...
                  <button
                    ...
                    onClick={sendOtp}
                    className="flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 px-5 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {sendingOtp && (
            ...
            {step === "otp" && (
              <form onSubmit={verifyOtp} className="space-y-5">
                <p className="rounded-xl bg-emerald-50 px-4 py-3 text-center text-sm font-black text-emerald-700">
                  {email}
                </p>
                ...
                <div className="grid grid-cols-6 gap-2">
                  {otp.map((digit, index) => (
                    <input
                      ...
                      className="h-12 rounded-xl border border-slate-200 bg-white text-center text-lg font-black outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/15"
                    />
                  ))}
                </div>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold outline-none transition placeholder:text-slate-300 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/15"
      />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/15"
      >
        {children}
      </select>
    </label>
    ...
    <label className="block">
      ...
      <div className="relative">
        <input
          ...
          value={value}
          onChange={(event) => onChange(event.target.value)}
          className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 pr-11 text-sm font-bold outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/15"
        />

        <button
          type="button"
          onClick={() => setShow(!show)}
          className="absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-slate-500 transition hover:bg-emerald-50 hover:text-emerald-700"
        >
