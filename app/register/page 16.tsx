...
// COMMENT: Added multilingual focus warning hints for Name, Father Name, Roll Number, Email, and Password
const text = {
  en: {
    ...
    rollNumberHint:
      "Warning: Usually starts with 5CS-, 5CT-, 5CS1-, or 5CT1-",
  ...
  mm: {
    ...
    nrc: "မှတ်ပုံတင်အမှတ်",
    region: "ကုဒ်",
    township: "မြို့နယ်ကုဒ်",
    nrcType: "အမျိုးအစား",
    nrcNumber: "အမှတ်စဉ်နံပါတ်",
    ...
    rollNumberHint:
      "သတိပြုရန် - 5CS-, 5CT-, 5CS1-, 5CT1- ဖြင့် စရေးရမည် ။",
    emailHint: "OTP စစ်ဆေးရန်အတွက် လက်ရှိအသုံးပြုနေသည的 အီးမေးလ်ကို ဖြည့်ပါ။",
    passwordHint:
...
export default function RegisterPage() {
  ...
  // COMMENT: Dynamically generate Graduated Year options from 2007 to (currentYear + 1) in descending order
  ...
  // COMMENT: Notice Roll Number is ONLY checked for non-empty string (`rollNumber.trim()`).
  // We explicitly avoid checking strict regex (like starting with 5CS- or 5CT-) because database entries
  // sometimes start with variations like 5CS1- or 5CT1-. Only a helpful focus warning is shown!
  const approvalValid =
    isNameValid(name) &&
    ...
    try {
      const res = await fetch("/api/register/check-approved", {
        ...
        body: JSON.stringify({
          ...
          normalizedNrcMm: normalizeForMatch(nrcValueMm),
          normalizedNrcEn: normalizeForMatch(nrcValueEn),
  ...
  return (
    <main className="min-h-[calc(100vh-70px)] px-2 pb-6 pt-6 sm:px-3">
      <section className="relative mx-auto grid max-w-7xl overflow-hidden rounded-2xl border border-white/20 bg-gradient-to-br from-[#3a6968] via-white to-[#eaffff] shadow-md lg:grid-cols-[0.95fr_1.05fr]">
        ...
        {/* COMMENT: Updated Left Column class from "flex" to "hidden lg:flex" so it disappears on mobile views */}
            ...
            {step === "approval" && (
              <div className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  {/* COMMENT: Roll Number input with onFocus triggering 7-second warning (starts with 5CS/5CT).
                      No strict blocking validation applied because db records also have 5CS1/5CT1 etc. */}
                  <Input
                    label={t.rollNumber}
                    value={rollNumber}
                    placeholder="5CS-1 or 5CT-1"
                    onChange={setRollNumber}
                    onFocus={() => setActiveHint("rollNumber")}
                    hint={activeHint === "rollNumber" ? t.rollNumberHint : ""}
                  />
