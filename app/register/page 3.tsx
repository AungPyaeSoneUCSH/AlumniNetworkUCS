...
export default function RegisterPage() {
  ...
  const [checkedStudentId, setCheckedStudentId] = useState(`${PREFIX} ----`);
  ...
  const showNumberError = focused.number && numberError;
  ...
  useEffect(() => {
    setApproved(false);
    setApprovalMessage("");
    setCheckedStudentId(`${PREFIX} ${number.trim() || "----"}`);
  }, [name, number]);
  ...
  async function checkApprovedStudent() {
    ...
    if (nameError) {
      setApprovalMessage(nameError);
      return;
    }

    if (numberError) {
      setApprovalMessage(numberError);
      return;
    }
    ...
    if (!name.trim() || !number.trim()) {
      setApprovalMessage(
        lang === "mm"
          ? "အမည်နှင့် တက္ကသိုလ်မှတ်ပုံတင်အမှတ် ထည့်ပါ။"
          : "Enter name and university register number."
      );
      return;
    }
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-white/60 bg-white/80 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
        ...
        {step === "approval" && (
          <>
            <Header
              ...
              title={lang === "mm" ? "Admin Approval စစ်မည်" : "Check Admin Approval"}
              description={
                lang === "mm"
                  ? "အမည်နှင့် တက္ကသိုလ်မှတ်ပုံတင်အမှတ် ကိုက်ညီမှသာ Register ဆက်လုပ်နိုင်ပါသည်။"
            ...
            <div className="relative space-y-5">
              ...
              <StudentNumberInput
                ...
                error={showNumberError ? numberError : ""}
                lang={lang}
                onFocus={() =>
              ...
              <button
                ...
                className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3.5 text-sm font-black tracking-wide text-white shadow-lg transition hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50"
              >
                {approvalChecking ? (
        ...
        {step === "info" && (
          <>
            <Header
              ...
              title={lang === "mm" ? "Register ဆက်လုပ်မည်" : "Continue Register"}
              description={
                lang === "mm"
                  ? "Admin approval အောင်မြင်ပါသည်။ Email နှင့် Password ထည့်ပါ။"
                  : "Approved. You can register now. Enter email and password."
              }
            />

            <div className="relative mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-center text-sm font-black text-emerald-700 shadow-sm">
              {checkedStudentId}
            </div>
        ...
        {step === "otp" && (
          <>
            ...
            <div className="relative mb-3 rounded-2xl border border-[#25C9C8]/30 bg-[#F8FFFF] px-4 py-3 text-center text-sm font-black text-[#008B8B] shadow-sm">
              {checkedStudentId}
            </div>
...
function StudentNumberInput({
  ...
  return (
    <div className="space-y-2">
      ...
      <div
        className={`group flex overflow-hidden rounded-2xl border bg-[#F8FFFF] shadow-sm transition focus-within:bg-white focus-within:ring-4 ${
          error
            ? "border-red-300 focus-within:border-red-400 focus-within:ring-red-100"
            : "border-[#25C9C8]/30 focus-within:border-[#00BFC4] focus-within:ring-[#00BFC4]/10"
        }`}
      >
        <span className="flex shrink-0 items-center gap-2 border-r border-[#25C9C8]/20 px-4 text-xs font-black text-[#008B8B]">
          <IdCard size={16} />
          {PREFIX}
        </span>
        ...
        <input
          ...
          placeholder="2532"
          required
          className="min-w-0 flex-1 bg-transparent px-4 py-3.5 text-sm font-bold text-slate-800 outline-none placeholder:text-slate-400"
        />
      </div>

      {error && <FieldError message={error} />}

      <div className="rounded-2xl border border-[#25C9C8]/20 bg-[#F8FFFF] px-4 py-3 text-xs font-black text-[#008B8B]">
        Student ID: {checkedStudentId}
      </div>
    </div>
  );
}
...
function ApprovalBox({
  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-2xl border px-4 py-3 text-sm font-black shadow-sm ${
        approved
