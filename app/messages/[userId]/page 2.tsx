...
import { ArrowLeft, Send, Trash2, UserRound } from "lucide-react";
  ...
  return (
    <main className="mm fixed inset-x-0 bottom-0 top-[72px] overflow-hidden bg-white text-[var(--ucsh-text)] sm:top-[80px]">
      <div className="flex h-full w-full flex-col overflow-hidden bg-white">
        <header className="flex h-[56px] shrink-0 items-center gap-2 border-b border-slate-200 bg-white px-2 sm:h-[60px] sm:px-3">
          <Link
            href="/messages"
        ...
        <section
          ref={chatBodyRef}
          className="flex-1 overflow-y-auto bg-white px-2 py-3 sm:px-4"
        >
          {messages.length === 0 ? (
            ...
            <div className="space-y-1">
                    ...
                    >
                      {!mine && (
                        <Image
                          ...
                          width={34}
                          height={34}
                          className="mb-4 h-[34px] w-[34px] rounded-full object-cover"
                        />
                      )}

                      <div
                        className={`flex max-w-[82%] items-end gap-1.5 sm:max-w-[62%] ${
                          mine ? "flex-row" : "flex-row-reverse"
                        }`}
                      >
                        <span className="mb-1 shrink-0 text-[11px] font-semibold text-slate-400">
                          {formatTime(message.createdAt)}
                        </span>

                        <div
                          className={`relative rounded-2xl px-3.5 py-2 text-sm leading-6 shadow-sm ${
                            mine
                              ? "rounded-br-sm bg-[#baf59f] text-slate-900"
                        ...
                        >
                          ...
                          {mine && (
                            <button
                              ...
                              className="absolute -left-9 top-1/2 hidden h-7 w-7 -translate-y-1/2 items-center justify-center rounded-lg bg-white text-red-500 shadow transition hover:bg-red-50 disabled:opacity-50 group-hover:flex"
                            >
                              <Trash2 size={14} />
                            </button>
                          )}
              ...
              {sending && (
                <div className="flex justify-end">
                  <div className="rounded-2xl rounded-br-sm bg-[#baf59f] px-4 py-3 shadow-sm">
