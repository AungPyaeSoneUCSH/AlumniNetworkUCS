...
export default function MessagesPage() {
  ...
  async function sendMessage() {
    ...
    // RULE ENFORCEMENT: Nobody (neither user nor admin) can send messages to an Admin.
    ...
    if (isTargetAdmin) {
      setDialog({
        ...
        description: "Direct messaging to Administrator accounts is not permitted.",
        type: "alert",
      });
      return;
    }
   ...
   * 1. Exclude current logged-in user from the chat list.
   * 2. Hide ALL Admin accounts from the chat list:
   *    - Regular users cannot see Admins (so they cannot message them).
  ...
  /**
   * UI RESTRICTIONS:
   * If the selected user is an Admin (e.g. loaded via storage or URL bypass),
   * lock the chat input so messages cannot be sent.
   */
              ...
              <footer className="shrink-0 border-t border-[var(--ucsh-border)] bg-white/85 p-2.5 backdrop-blur-xl dark:bg-slate-950/85 sm:p-3">
                {isChatRestricted ? (
                  <div className="flex items-center justify-center gap-2 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs font-black text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-300">
                    ...
                    <span>Direct messaging to Administrator accounts is not permitted.</span>
                  </div>
                ) : (
