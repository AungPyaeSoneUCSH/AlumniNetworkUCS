...
import { ArrowLeft, MessageCircle, Search, Send, UserRound, ShieldAlert, Edit2, Trash2, Check, X, Ban } from "lucide-react";
...
export default function MessagesPage() {
  ...
  async function sendMessage() {
    ...
    if (!amIAdmin && isTargetAdmin) {
      alert("You are not authorized to send messages to an Administrator.");
      return;
    }
                            ...
                            <div className="relative flex items-center">
                              ...
                              {isMine && !message.isDeleted && !isEditingThis && (
                                <div className="absolute -left-16 top-1/2 z-10 hidden -translate-y-1/2 items-center gap-1 rounded-xl border border-[var(--ucsh-border)] bg-white p-1 shadow-md group-hover:flex dark:bg-slate-900">
                                  ...
                                  <button type="button" onClick={() => deleteMessage(message._id)} disabled={actionLoadingId === message._id} title="Delete" className="rounded-lg p-1 text-slate-500 transition hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/50"><Trash2 size={13} /></button>
                                </div>
                              )}
                            </div>
                          </div>
