...
const text = {
  en: {
    ...
    recentPostsText: "Top 2 posts within this month",
    noRecentPosts: "No recent posts this month.",
    all: "All",
  },
  mm: {
    ...
    calendar: "ပြက္ခဒိန်",
    recentPosts: "လတ်တလော Posts",
    recentPostsText: "ဒီလအတွင်း top 2 posts",
    noRecentPosts: "ဒီလအတွင်း post မရှိသေးပါ။",
    all: "အားလုံး",
  },
};
...
export default function FeedsPage() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter((post) => {
      ...
      const matchAuthor = filterAuthor ? post.author?._id === filterAuthor : true;
      const matchDate = selectedDate ? toDateKey(post.createdAt) === selectedDate : true;
  ...
  const recentPosts = useMemo(() => {
    return posts
      .filter((post) => isWithinCurrentMonth(post.createdAt))
      .sort(
        (a, b) =>
  ...
  const hasActiveFilter = Boolean(search || filterCategory || filterAuthor || selectedDate);

  return (
    <main className="mm page-wrapper relative overflow-hidden text-[var(--ucsh-text)]">
      <BackgroundDecor />

      <section className="ucsh-container relative z-10 max-w-7xl">
        <div className="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)_310px]">
          <aside id="page-search" className="scroll-mt-28 lg:sticky lg:top-24 lg:self-start">
            <FeedFilters
              search={search}
  ...
  return (
    <div className="ucsh-card ucsh-animate p-4">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--ucsh-muted)]">
            {t.filters}
          </p>
          <h2 className="text-lg font-black text-[var(--ucsh-text)]">
      ...
      <div className="relative">
        <Search
          size={17}
          className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[var(--ucsh-primary-dark)]"
        />
        <input
          value={search}
    ...
    >
      ...
      <span
        className={`rounded-full px-2.5 py-1 text-[11px] ${
          active ? "bg-white/20 text-white" : "bg-cyan-50 text-[var(--ucsh-primary-dark)]"
        }`}
      >
        {count}
      </span>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <h3 className="text-sm font-black text-[var(--ucsh-text)]">
              {month.toLocaleDateString(currentLang === "mm" ? "my-MM" : "en-US", {
                month: "long",
                year: "numeric",
              })}
            </h3>
  ...
  return (
    <div className="ucsh-card ucsh-animate p-4">
      <div className="mb-4">
        ...
        <h3 className="mt-1 text-sm font-black text-[var(--ucsh-text)]">
          {t.recentPostsText}
        </h3>
      </div>
...
function PostCard({
  ...
  const postImages = Array.isArray(post.images) && post.images.length > 0 ? post.images : post.image ? [post.image] : [];
  ...
  return (
    ...
    >
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 line-clamp-1 text-xs font-bold text-[var(--ucsh-muted)]">
              ...
              {post.author?.graduatedYear ? ` • ${post.author.graduatedYear}` : ""}
            </p>
        ...
        {postImages.length > 0 && (
          <div className={`mt-4 grid gap-2 ${postImages.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}>
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 border-t border-[var(--ucsh-border)] pt-4">
          <ActionButton
            active={liked}
            icon={<ThumbsUp size={18} className={liked ? "fill-[#1877F2] text-[#1877F2]" : ""} />}
