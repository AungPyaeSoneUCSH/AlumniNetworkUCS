...
export default function FeedsPage() {
  ...
  const [filterCategory, setFilterCategory] = useState(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("feed_filter_category") || "";
    }
    return "";
  });
  const [filterAuthor, setFilterAuthor] = useState(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("feed_filter_author") || "";
    }
    return "";
  });
  const [selectedDate, setSelectedDate] = useState(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("feed_filter_date") || "";
    }
    return "";
  });
  ...
  useEffect(() => {
    sessionStorage.setItem("feed_filter_search", search);
  }, [search]);

  useEffect(() => {
    sessionStorage.setItem("feed_filter_category", filterCategory);
  }, [filterCategory]);

  useEffect(() => {
    sessionStorage.setItem("feed_filter_author", filterAuthor);
  }, [filterAuthor]);

  useEffect(() => {
    sessionStorage.setItem("feed_filter_date", selectedDate);
  }, [selectedDate]);
  ...
  function clearFilters() {
    ...
    if (typeof window !== "undefined") {
      ...
      sessionStorage.removeItem("feed_filter_author");
      sessionStorage.removeItem("feed_filter_date");
    }
  }
